...
% Check whether the first token is \#.  If yes, call \autoref, otherwise the
% original \href.  \@nil is a delimiter token which we need because we call
% the arguments in \modified@href without braces.
\def\href#1{%
  \modified@href #1\@nil}
\def\modified@href#1#2\@nil{%
  ...
  \fi
  {#1}{#2}}
% We normalize \_ in the reference (#2), which we put it into parentheses
% after the label (#3).  Argument #1 holds \# which we skip.
